...
for /f "tokens=2*" %%a in ('REG QUERY HKCU\Software\Microsoft\Windows\CurrentVersion\GameDVR /v AudioCaptureEnabled') do set "regAudioCaptureEnabled=%%b"
...
if %regAudioCaptureEnabled% == 0x1 set "DVR_ENEBELED=1"
...
reg export HKCU\Software\Microsoft\Windows\CurrentVersion\GameDVR "%regbackUpTEMPFile%" /y&& type "%regbackUpTEMPFile%" | findstr "AppCaptureEnabled">>"%regbackUpFile%"
reg export HKCU\Software\Microsoft\Windows\CurrentVersion\GameDVR "%regbackUpTEMPFile%" /y&& type "%regbackUpTEMPFile%" | findstr "AudioCaptureEnabled">>"%regbackUpFile%"&echo.>>"%regbackUpFile%"
...
for /f "tokens=2*" %%a in ('REG QUERY HKCU\Software\Microsoft\Windows\CurrentVersion\GameDVR /v AudioCaptureEnabled') do set "cregAudioCaptureEnabled=%%b"
...
if %cregAudioCaptureEnabled% == 0x1 set "CDVR_ENEBELED=1"
